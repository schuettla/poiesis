...
/// Filename prefix of sd.cpp's own engine assets (`sd-master-…-bin-win-…`),
/// which is what keeps them apart from the `cudart-sd-…` DLL package.
pub const SD_ENGINE_PREFIX: &str = "sd-";
/// Filename prefix of the CUDA runtime DLL package shipped beside them.
pub const SD_CUDART_PREFIX: &str = "cudart-";
...
/// applied on top of [`SD_CUDART_PREFIX`].
pub fn sd_cudart_keywords(backend: Backend) -> Option<Vec<&'static str>> {
    matches!(backend, Backend::Cuda12 | Backend::Cuda13).then(|| vec!["cu12", "x64"])
}
...
mod tests {
    ...
    fn matches(name: &str, prefix: &str, kw: &[&str]) -> bool {
        let l = name.to_ascii_lowercase();
        l.starts_with(prefix) && l.ends_with(".zip") && kw.iter().all(|k| l.contains(k))
    }

    fn pick<'a>(prefix: &str, kw: &[&str]) -> Option<&'a str> {
        SD_WIN_ASSETS.iter().copied().find(|n| matches(n, prefix, kw))
    }

    fn engine<'a>(backend: Backend) -> Option<&'a str> {
        pick(SD_ENGINE_PREFIX, &sd_asset_keywords(backend))
    }

    #[test]
    fn resolves_backends_to_real_sd_assets() {
        assert_eq!(engine(Backend::Cuda12), Some("sd-master-484baa4-bin-win-cuda12-x64.zip"));
        assert_eq!(engine(Backend::Vulkan), Some("sd-master-484baa4-bin-win-vulkan-x64.zip"));
        assert_eq!(engine(Backend::Cpu), Some("sd-master-484baa4-bin-win-cpu-x64.zip"));
        assert_eq!(engine(Backend::Hip), Some("sd-master-484baa4-bin-win-rocm-7.1.1-x64.zip"));
    }

    #[test]
    fn cuda_engine_does_not_match_the_cudart_package() {
        let got = engine(Backend::Cuda12).unwrap();
        assert!(!got.starts_with("cudart"), "matched the cudart package: {got}");
        let kw = sd_cudart_keywords(Backend::Cuda12).unwrap();
        assert_eq!(pick(SD_CUDART_PREFIX, &kw), Some("cudart-sd-bin-win-cu12-x64.zip"));
    }
}
